...
def seismic_data(client1, 
    ...
    # ----------------------------------------------------------------------
    station_data.write('station_data.mseed', format='mseed')  
...
def apply_filter(wave_dict, 
    ...
    for station_name in wave_dict:
        ...
        wave_dict[station_name] = st.traces # Update the traces after filtering
...
def apply_agc(wave_dict, 
    ...
    for station in wave_dict:
        for agc_trace in wave_dict[station]:
...
def polar_plot(wave_dict, 
    ...
    for i, station in enumerate(station_list, start=1):
        ...
        # Gather peaks
        peaks, _ = find_peaks(r) # find_peaks(r) returns two values. Only the first is needed

        if len(peaks) > 0: 
            first_peak = peaks[0] # Select first peak after time zero            
            first_peak_angle_deg = theta_deg[first_peak]
            print(f"{station}: first peak at angle = {first_peak_angle_deg:.2f}°") # Print angle of first peak 
        else:
            first_peak = None
...
def tabulate_peaks(wave_dict, 
    ...
    for i, station in enumerate(station_list, start=1):
        ...
        angle_results.append({"Station": station,"Seismic Direction (deg)": first_peak_angle_deg})

    # Tabulate Angles
    df = pd.DataFrame(angle_results)
    df.to_csv('seismic_directions.csv', index=False)
    print("\Seismic Direction:")
...
def polar_correction(wave_dict, 
    ...
    for i, station in enumerate(station_list, start=1):
        ...
        # Angle correction
        if first_peak is not None:
            correction_angle = theta[first_peak] - ref_angle
        else:
        ...
        if first_peak is not None:
            ax.plot(theta[first_peak], 
                    ...
                    markersize=8, 
                    label="First Peak")
            ax.plot(theta[first_peak] - correction_angle, 
                    r[first_peak], 
                    'ro', 
                    color = 'red',
                    markersize=8, 
                    label="First Peak Corrected")
...
def waveform_and_polar(wave_dict,  
    ...
    for i, station in enumerate(station_list, start=1):
        ...
        if first_peak is not None:
            ...
            offset = 0.25 * rmax  # Offset for cardinal labels
